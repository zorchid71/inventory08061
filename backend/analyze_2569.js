...
async function run() {
  const filePath = path.join(__dirname, '..', 'y2569.xlsx');
  let workbook = xlsx.readFile(filePath);
  
  const sheetNames = workbook.SheetNames;
  console.log(`y2569.xlsx มีทั้งหมด ${sheetNames.length} Sheet ได้แก่: ${sheetNames.join(', ')}`);
  ...
  let errors = [];
  let previousMonthBalances = {};

  // 1. Check Linkage between Sep 68 (from SQLite) and Oct 68 (first sheet of 2569)
  const sep68Records = await sqliteQuery.all(`
    SELECT i.name, s.remaining_qty 
    FROM non_drug_monthly_stock s
    JOIN non_drug_items i ON s.item_id = i.id
    WHERE s.fiscal_year = 2568 AND s.month_name = 'ก.ย.68'
  `);
  const sep68Balances = {};
  sep68Records.forEach(r => { sep68Balances[r.name] = r.remaining_qty; });
  ...
  for (let sIdx = 0; sIdx < sheetNames.length; sIdx++) {
    ...
    for (let i = headerRowIdx + 1; i < data.length; i++) {
      ...
      if (sIdx === 0) {
        const prevBal = sep68Balances[cleanName];
        if (prevBal !== undefined && Math.abs(prevBal - beg) > 0.01) {
          errors.push(`[${sheetName}] Row ${i+1}: "${cleanName}" ยกมา(${beg}) ไม่ตรงกับ คงเหลือ ก.ย.68 ในระบบ(${prevBal})`);
        }
      } else {
  ...
  const fs = require('fs');
  const outPath = path.join(__dirname, '..', 'analysis_y2569.md');
  let md = `# ผลการวิเคราะห์ข้อมูลไฟล์ y2569.xlsx\n\n`;
  ...
  console.log('Analysis completed. See analysis_y2569.md');
  process.exit(0);
}
