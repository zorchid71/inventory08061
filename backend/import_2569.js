...
async function run() {
  ...
  for (let sIdx = 0; sIdx < sheetNames.length; sIdx++) {
    ...
    if (headerRowIdx === -1) {
      console.error(`[${sheetName}] หาแถว Header (รายการยา) ไม่พบ ข้าม Sheet นี้`);
      continue;
    }
    ...
    for (let i = headerRowIdx + 1; i < data.length; i++) {
      ...
      // Insert or Update Item in Master List to keep it up-to-date
      let itemRow = await sqliteQuery.get(`SELECT id FROM non_drug_items WHERE name = ?`, [cleanName]);
      if (!itemRow) {
        ...
        `, [
          cleanName, 
          row[2] || '', // ประเภทยา (actually unit/type)
          row[3] || '', // ขนาดบรรจุ
          price, 
          null
        ]);
        itemRow = await sqliteQuery.get(`SELECT id FROM non_drug_items WHERE name = ?`, [cleanName]);
      } else {
        // Update price just in case it changed
        await sqliteQuery.run(`UPDATE non_drug_items SET unit_price = ? WHERE id = ?`, [price, itemRow.id]);
      }

      // Upsert monthly stock
      await sqliteQuery.run(`
  ...
  console.log(`Done! Total 2569 monthly records imported/updated: ${totalImported}`);
  process.exit(0);
}
